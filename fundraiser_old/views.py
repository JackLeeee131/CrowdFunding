            ...
            if request.method == "POST":
                ...
                if form_type == 'Goal':
                    ...
                    if Campaign_Comments_Form.is_valid():
                        ...
                        messages.add_message(request,messages.SUCCESS,'Coment added successfully.')
...
def support_group_selected(request, ID, name):
    try:
        ...
        if is_authorised_for_edit:
            ...
            instance_SupportGroupMembers = SupportGroupMembers.objects.filter(support_group=instance_SupportGroup, is_show_name=True).order_by("-id")
...
def admin_manage_category(request):
    form_error = False
    if request.method == 'POST':
        Campaign_Category_Form = CampaignCategoryForm(request.POST)
...
def admin_manage_category_edit(request, ID):
    try:
        instance_CampaignCategory = CampaignCategory.objects.get(id=ID)
        if request.method == 'POST':
            Campaign_Category_Form = CampaignCategoryForm(request.POST, instance=instance_CampaignCategory)
            if Campaign_Category_Form.is_valid():
                Campaign_Category_Form = Campaign_Category_Form.save()
...
def admin_manage_user(request):
    ...
    if request.method == 'POST':
        ...
        if Admin_User_Signup_Form.is_valid():
            ...
            messages.add_message(request,messages.SUCCESS,'User "%s" updated successfully' %(Admin_User_Signup_Form.name))
